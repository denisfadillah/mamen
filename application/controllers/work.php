...
class Work extends BaseController
{
    ...
    function workListing()
    {
        ...
        {        
            ...
            $count = $this->work_model->workListingCount($searchText);

			$returns = $this->paginationCompress ( "workListing/", $count, 5 );
            
            $data['workRecords'] = $this->work_model->workListing($searchText, $returns["page"], $returns["segment"]);
            
            $this->global['pageTitle'] = 'CodeInsect : work Listing';
            
            $this->loadViews("admin/works", $this->global, $data, NULL);
        }
    }
    ...
    {
		$this->load->model('work_model');		
		$this->global['pageTitle'] = 'CodeInsect : Add New Work';
		$data = [];
		$this->loadViews("admin/addNewWork", $this->global, $data, NULL);
    }	
